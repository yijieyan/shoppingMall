            ...
            <ul class="cart-item-list">
              <li v-for="item in cartList">
                <div class="cart-tab-1">
                  <div class="cart-item-check">
                    <a href="javascipt:;" class="checkbox-btn item-check-btn"
                       v-bind:class="{'check':item.checked== true}" @click="edit(item, 'checked')">
                ...
                <div class="cart-tab-3">
                  <a href="javascript:;" class="bt" @click="edit(item, 'sub')">-</a>
                  <span class="num">{{item.num}}</span>
                  <a href="javascript:;" class="bt" @click="edit(item, 'add')">+</a>
                </div>
                <div class="cart-tab-4">
                  <div class="item-price-total">{{item.price * item.num | currency('￥')}}</div>
                </div>
                <div class="cart-tab-5">
                  <div class="cart-item-opration" @click="deleteItem(item)">
        ...
        <div class="cart-foot-wrap">
          <div class="cart-foot-inner">
            <div class="cart-foot-l">
              <div class="cart-item-check">
                <a href="javascipt:;" class="checkbox-btn item-check-btn" v-bind:class="{'check':isSelectAll== true}"
                   @click="selectAll">
                  <svg class="icon icon-ok">
                    <use xlink:href="#icon-ok"></use>
                  </svg>
            ...
            <div class="cart-foot-r">
              ...
              <div class="btn-wrap">
                <button class="btn btn--red" @click="checkout" :disabled="cartList.length> 0 ? false : 'disabled'"
                        :class="cartList.length> 0 ? '' : 'disabled'">Checkout
                </button>
              </div>
            </div>
          </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      init() {
        http.get('/users/getMyGoodList').then((res) => {
          if (res.code == 0) {
      ...
      edit(item, flag) {
        ...
        http.post('/users/updateGoodCount', {id: item.goodId, num: item.num, checked: item.checked}).then((res) => {
          if (res.code == 0) {
      ...
      selectAll() {
        ...
        http.post('/users/selectAllGood', {flag: this.isSelectAll}).then((res) => {
          if (res.code == 0) {
            console.log(res.data);
          }
        })
      ...
      deleteItem(item) {
        http.post('/users/deleteGood', {id: item.goodId}).then((res) => {
          if (res.code == 0) {
            console.log(res.data);
            this.init();
          }
