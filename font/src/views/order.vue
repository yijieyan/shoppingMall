      ...
      <div class="checkout-order">
              ...
              <li v-for="(item, index) in goods" :key="index">
                <div class="cart-tab-1">
                  <div class="cart-item-pic">
                    <img :src="'/static/' + item.pic" :alt="item.productName">
                  </div>
                ...
                <div class="cart-tab-3" style="padding-top: 57px;">
        ...
        <div class="order-foot-wrap">
          <div class="prev-btn-wrap">
            <a class="btn btn--m" @click="goBack">Previous</a>
          </div>
          <div class="next-btn-wrap">
            <button class="btn btn--m btn--red" @click="nextToPage">Proceed to payment</button>
          </div>
        </div>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      init() {
        http.get('/users/getOrder').then((res) => {
          if (res.code == 0) {
            this.goods = res.data;
      ...
      nextToPage() {
        http.post('/users/comfirmOrder').then((res) => {
          if (res.code == 0) {
            this.$router.push({path: '/orderSuccess', query: {id: this.id}});
          }
        });
      },
      goBack() {
