<template>
  <div class="box">
      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                <li v-for="(item, index) in addressList" :key="index" @click="setDefault(item, index)">
                  <dl>
                    <dt>{{item.name}}</dt>
                    <dd class="address">{{item.address}}</dd>
                    <dd class="tel">{{item.phone}}</dd>
                  </dl>
                  <div class="addr-opration addr-del" @click="deleteItem(item)">
                  ...
                  <div class="addr-opration addr-default" v-if="defaultIndex == index ">Default address</div>
                </li>
    ...
    <modal v-if="showAddressModal" @closeModal="closeModal" :mdShow="showAddressModal">
      <div slot="message">
        <label for="name" class="label">
          <input type="text" id="name" placeholder="收件人" class="input" v-model="name"/>
        </label>
        <label for="address" class="label">
          <input type="text" id="address" placeholder="请输入您的收货地址" class="input" v-model="address"/>
        </label>
        <label for="phone" class="label">
          <input type="text" id="phone" placeholder="请输入您的电话" class="input" v-model="phone"/>
        </label>
      </div>
      <div slot="btnGroup">
        <a href="javascript:;" class="btn" @click="closeModal">关闭</a>
        <a href="javascript:;" class="btn" @click="confirmAddress">确定</a>
      </div>
    </modal>
    <navFooter></navFooter>
  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      init() {
        http.post('/users/getAddress').then((res) => {
          if (res.code == 0) {
            res.data.myAddressList.forEach((item, index) => {
              if (item.isDefault == true) {
                this.defaultIndex = index;
              }
            });
            this.list = JSON.parse(JSON.stringify(res.data.myAddressList));
            this.addressList = res.data.myAddressList.splice(0, this.count);
          }
        })
      },
      setDefault(item, index) {
        http.post('/users/setDefault', {name: item.name}).then((res) => {
          if (res.code == 0) {
            this.init();
          }
        });
      ...
      confirmAddress() {
        http.post('/users/confirmAddress', {name: this.name, address: this.address, phone: this.phone}).then((res) => {
          if (res.code == 0) {
            this.showAddressModal = false;
            this.init();
          }
      ...
      deleteItem(item) {
        http.post('/users/deleteAddress', {name: item.name}).then((res) => {
          if (res.code == 0) {
            this.init();
          }
        })
      ...
      next() {
        http.post('/order/commitOrder', {addressInfo: JSON.stringify(this.list[this.defaultIndex])}).then((res) => {
          if (res.code == 0) {
            this.$router.push({path: '/order', query: {id: res.data._id}});
          }
        })
      }
    }
